...
public class PerformTransactionService {
	...
	public void performTransaction(long acc, TransactionDetails trans1) throws BankManagementException {
		...
		if (trans1.getTransactionType().equals("withdrawal")) {
			if (trans1.getTransactionAmount() > ud.getAccountBalance()) {
				throw new BankManagementException(
						"transactionAmount:AccountBalance should be greater than TransactionAmount");
			}

		}

		if (!trans1.getTransactionType().equals("deposit") && !trans1.getTransactionType().equals("withdrawal")) {
			throw new BankManagementException("transactionType:Transaction Type should be Deposit or Withdrawal");
		}

		else if (trans1.getTransactionType().equals("withdrawal")) {
			if (trans1.getTransactionAmount() > ud.getAccountBalance()) {
				throw new BankManagementException(
						"transactionAmount:Withdrawal amount is greater than account balance");
			} else if (ud.getAccountType().equalsIgnoreCase("savings")) {

				if ((ud.getAccountBalance() - trans1.getTransactionAmount()) <= 5000)
					throw new BankManagementException("transactionAmount:Savings account balance must be above 5000");
			}

		}

		else if (ud.getAccountType().equals("salary")) {
			if (ud.getAccountBalance() < 0) {
				throw new BankManagementException("transactionAmount:Minimum  balance in salary account should be 0");
			}
		}
	...
	public void addSingleUser(UserDetails user1) {
		...
		} else if (user1.getAccountType().equals("salary")) {
